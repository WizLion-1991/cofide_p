...
<%@page import="Tablas.vta_ncreditodeta"%>
   ...
   if (varSesiones.getIntNoUser() != 0 && seg.ValidaURL(request)) {
      ...
      if (strid != null) {
         ...
         if (strid.equals("1")) {
            ...
            NCredito ncredito = new NCredito(oConn, varSesiones, request);
            ...
            ncredito.getDocument().setFieldInt("CT_ID", Integer.valueOf(request.getParameter("CT_ID")));
            ...
            try {
               ...
               System.out.println("ERP_NCREDITO TI_ID " + ex.getMessage());
            }
            //Tipo de comprobante
            ...
            //Sucursal destino.
            if (request.getParameter(strPrefijoMaster + "_SC_ID2") != null) {
            ...
            if (request.getParameter("FAC_ID") != null) {
               ncredito.getDocument().setFieldInt("FAC_ID", Integer.valueOf(request.getParameter("FAC_ID")));
            }
            //Recibimos el ID de la remisión
            if (request.getParameter("TKT_ID") != null) {
               ncredito.getDocument().setFieldInt("TKT_ID", Integer.valueOf(request.getParameter("TKT_ID")));
            }
            //Recibimos bandera para no timbrar
            if (request.getParameter("NC_ES_DEVO") != null) {
               if(request.getParameter("NC_ES_DEVO").equals("0")){
                  ncredito.setBolTimbrar(true);
                  ncredito.getDocument().setFieldInt(strPrefijoMaster + "_ES_DEVO", 0);
               }else{
                  ncredito.setBolTimbrar(false);
                  ncredito.getDocument().setFieldInt(strPrefijoMaster + "_ES_DEVO", 1);
               }
            }
            //Validamos si tenemos un empresa seleccionada
            if (varSesiones.getIntIdEmpresa() != 0) {
            ...
            for (int i = 1; i <= intCount; i++) {
               TableMaster deta = new vta_ncreditodeta();
               ...
               try {
                  ...
                  if (ncredito.getDocument().getFieldInt("FAC_ID") != 0) {
                     deta.setFieldInt("FACD_ID", Integer.valueOf(request.getParameter("FACD_ID" + i)));
                  }
                  if (ncredito.getDocument().getFieldInt("TKT_ID") != 0) {
                     deta.setFieldInt("TKTD_ID", Integer.valueOf(request.getParameter("FACD_ID" + i)));
                  }
         ...
         if (strid.equals("6")) {
            ...
            strFAC_ID = request.getParameter("TKT_ID");
            ...
            for (int iFac = 0; iFac < lstFacturas.length; iFac++) {
               ...
               ticket.ObtenDatos(Integer.valueOf(lstFacturas[iFac]), oConn);
               String strValorPar = ticket.getFieldPar();
               strXML.append(strValorPar + " > ");
               //Obtenemos el detalle
               vta_ticketsdeta deta = new vta_ticketsdeta();
               ArrayList<TableMaster> lstDeta = deta.ObtenDatosVarios(" TKT_ID = " + ticket.getFieldInt("TKT_ID"), oConn);
               Iterator<TableMaster> it = lstDeta.iterator();
               ResultSet rs = null;
               String strSql;
               while (it.hasNext()) {
                  ...
                  int intCantDev = 0;
                  //Recuperamos los numeros devueltos s es que los hay
                  String strSqlDev = "SELECT TKTD_CAN_DEV,TKTD_SERIES_DEV "
                          + "  FROM vta_ticketsdeta deta "
                          + "  WHERE deta.TKTD_ID= " + tbn.getFieldInt("TKTD_ID");

                  rs = oConn.runQuery(strSqlDev, true);
                  while (rs.next()) {
                     strSeriesDev = rs.getString("TKTD_SERIES_DEV");
                     intCantDev = rs.getInt("TKTD_CAN_DEV");
                  }
                  String numerosSeries = strSeriesDev.replace(",", "','");
                  ...
                  if (intPR_USO_NOSERIE == 1) {
                     ...
                     //Consulta de movimientos de producto ligados a este pedido
                     String strSqlProd = "SELECT MPD_ID,PL_NUMLOTE,MPD_SERIE_VENDIDO FROM vta_movproddeta "
                             + "WHERE MPD_IDORIGEN = " + tbn.getFieldInt("TKTD_ID") + " AND MPD_SALIDAS > 0 AND PL_NUMLOTE NOT IN('" + numerosSeries + "')";
                     rs = oConn.runQuery(strSqlProd, true);
                     while (rs.next()) {
                        int intMPD_ID = rs.getInt("MPD_ID");
                        String strPL_NUMLOTE = rs.getString("PL_NUMLOTE");
                     ...
                     //Si no encontro series entonces se surtido por un pedido
                     if (!bolFound) {
                        String strSqlProd2 = "SELECT vta_ticketsdeta.TKTD_NOSERIE,MPD_ID,PL_NUMLOTE,MPD_SERIE_VENDIDO FROM vta_movproddeta,vta_pedidosdeta,vta_ticketsdeta "
                                + " WHERE "
                                + " vta_movproddeta.MPD_IDORIGEN = vta_pedidosdeta.pdd_id  and "
                                + " vta_ticketsdeta.PDD_ID = vta_pedidosdeta.pdd_id and "
                                + " vta_ticketsdeta.TKTD_ID =  " + tbn.getFieldInt("TKTD_ID") + " AND MPD_SALIDAS > 0 AND PL_NUMLOTE NOT IN('" + numerosSeries + "')";
                        rs = oConn.runQuery(strSqlProd2, true);
                        while (rs.next()) {
                           int intMPD_ID = rs.getInt("MPD_ID");
                           String strPL_NUMLOTE = rs.getString("PL_NUMLOTE");
