...
<%
   ...
   if (!strPass.isEmpty()) {
      action.authentication_proveedor(oConn, strUser, strPass, request);
   }
   oConn.close();


   //Si tuvimos acceso mostramos la bienvenido sino mostramos error
   if (action.isBolTieneAcceso() && action.isBolAccesoProv()) {
      varSesiones.getVars();
...
%>
<center>
   <h1>BIENVENIDO PROVEEDOR <%=varSesiones.getStrUser()%></h1>
...
<%
   ...
   if (action.isBolErrorLogged()) {
      strMsgError = "EL PROVEEDOR YA HA INICIADO SESION";
   } else {
      if (action.isBolErrorBloqued()) {
         strMsgError = "EL PROVEEDOR O PASSWORD NO COINCIDEN, VERIFIQUELOS";
      }
   }
%>
<center>
