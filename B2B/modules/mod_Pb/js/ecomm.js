...
function guardarCompra() {
   ...
   for (var j = 0; j <= intContaItems; j++) {
      ...
      strHtml += "<td align='right'>" + FormatNumber(objItem.pr_vnegocio_tot, intNumdecimal, true) + "</td>";
        ...
        strHtml += "<td align='right'><input type=button onclick='EditarGrid(" + j + ")' value='Editar Cantidad'></td>";
      strHtml += "</tr>";
   }
   strHtml += "<tr>";
...
function EditarGrid(j) {

    $("#dialogWait2").dialog('option', 'title', "Edita Cantidad");
    var div = document.getElementById("dialogWait2");

    var intCantActual = 0;
    for (var l = 0; l <= intContaItems; l++) {
        var objItem = lstItems[l];
        if (j == l) {
            intCantActual = objItem.pr_cantidad;
        }
    }

    var strHtml = CreaTexto("", "pr_cantidad", intCantActual, 40, 40, true, false, "", "left", 0, "", "", "", false, 1);
    strHtml += CreaHidden("pr_cantidadAnerior", intCantActual);
    strHtml += CreaBoton("", "EDITBTN", "Aceptar", "EditarGrid2(" + j + ")", "center", false, false, 0);
    div.innerHTML = strHtml;
    $("#dialogWait2").dialog("open");

}

function EditarGrid2(j) {

    var intCantEditada = document.getElementById("pr_cantidad").value;
     var intCantAnterior = document.getElementById("pr_cantidadAnerior").value;
    ...
    for (var l = 0; l <= intContaItems; l++) {
        ...
        if (j != l) {
            ...
            objItem.pr_cantidad = intCantEditada;
            lstItemsTemp.push(objItem);
        }
    }
    lstItems = Array();
    ...
    intTotPzas = parseInt(intTotPzas) - parseInt(intCantAnterior); 
    intTotPzas = parseInt(intTotPzas) + parseInt(intCantEditada);
    ...
    //buscamos si existe el item en el listado
    for (var c = 0; c <= intContaItems; c++) {
        var objItem = lstItems[c];

        if (c == j) {
            //Ya existe solo actualizamos la cantidad
            objItem.pr_cantidad = parseFloat(objItem.pr_cantidad);
            var objImportes = new _ImporteVta();
            objImportes.dblCantidad = parseFloat(objItem.pr_cantidad);
            objImportes.dblPrecio = parseFloat(objItem.pr_precio);
            objImportes.dblPrecioReal = parseFloat(objItem.pr_precreal);
...
function SaveVtaDo() {
   ...
   $.ajax({
      ...
      success: function (dato) {
         ...
         var bolMercadoPago = document.getElementById("payment_1").checked;
         ...
         if (Left(dato, 3) == "OK.") {
            ...
            ImprimeconFolio(strKey, dato, strNomFormat, bolMercadoPago);
            limpiarCompra();
         } else {
            alert(dato);
         }
      ...
      success: function (datos) {
         ...
         if (bolMercadoPago == true) {
            $.ajax({
               ...
               cache: false,
               dataType: "xml",
               url: "modules/mod_klensy/ecomm_save.jsp?id=5",
               success: function (datos1) {

                  var objsc1 = datos1.getElementsByTagName("linkdeta")[0];
                  link = objsc1.getAttribute('link');
                  ...
                  document.getElementById("IdPedidoWeb").innerHTML = "<h3>" + strFolioT + "</h3>";
                        document.getElementById("link").innerHTML = "<a href=" + link + "  target='_blank' >De click aqui para realizar el pago <image src='" + document.getElementById("PAYMENT_IMAGE").value + "' border='0' alt='' width='371' height='12' ></a>";
            ...
            var strHtml2 = CreaHidden(strKey, dato.replace("OK.", ""));
            document.getElementById("IdPedidoWeb").innerHTML = "<h3>" + strFolioT + "</h3>";
            document.getElementById("link").innerHTML = "<h3>" + link + "</h3>";
            openFormatEcomm("PEDIDO", "PDF", strHtml2);
         }
      },
      error: function (objeto, quepaso, otroobj) {
...
function sumaTotalFleteTotal(dblValorFlete) {
   var oTotal = document.getElementById("FAC_TOT").value;
...
function prueba() {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "xml",
        url: "modules/mod_klensy/ecomm_save.jsp?id=6",
        success: function (datos1) {
