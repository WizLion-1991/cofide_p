 ...
 */
function vta_mmails() {//Funcion necesaria para que pueda cargarse la libreria en automatico
}

var bolAnularVta = false;
var strNomMainMM = false;

function InitVisMMails() {
   try {
...
function isTemplete() {
   if (document.getElementById("VIEW_TEMPLETE1").checked) {
      document.getElementById("VIEW_ASUNTO").setAttribute("class", "READONLY");
      document.getElementById("VIEW_ASUNTO").setAttribute("className", "READONLY");
      document.getElementById("VIEW_ASUNTO").readOnly = true;
      document.getElementById("VIEW_MAIL").setAttribute("class", "READONLY");
      document.getElementById("VIEW_MAIL").setAttribute("className", "READONLY");
      document.getElementById("VIEW_MAIL").readOnly = true;
   } else {
      document.getElementById("VIEW_ASUNTO").setAttribute("class", "outEdit");
      document.getElementById("VIEW_ASUNTO").setAttribute("className", "outEdit");
      document.getElementById("VIEW_ASUNTO").readOnly = false;
      document.getElementById("VIEW_MAIL").setAttribute("class", "outEdit");
      document.getElementById("VIEW_MAIL").setAttribute("className", "outEdit");
      document.getElementById("VIEW_MAIL").readOnly = false;
   }
}//Fin isTemplete
...
function ValidaCleanMM(strNomField) {
...
function ValidaShowMM(strNomField, strMsg) {
   var objDivErr = document.getElementById("err_" + strNomField);
   objDivErr.setAttribute("class", "");
...
function MMSendMail() {
   ...
   if (arr.length == 0) {
      ...
      $.ajax({
         type: "POST",
         data: encodeURI("LST_FAC_ID=" + strPD_ID +
                 ...
                 + "&VIEW_CC=" + intVIEW_CC
                 + "&VIEW_TEMPLETE=" + usaTempl)
         ,
         scriptCharset: "utf-8",
         contentType: "application/x-www-form-urlencoded;charset=utf-8",
         cache: false,
         dataType: "xml",
         url: "ERP_MailMasivo.jsp?id=1",
