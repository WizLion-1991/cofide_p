...
function RefreshClas(obj, optClas) {
    ...
    if (optClas != 0) {
        ...
        $.ajax({
            ...
            cache: false,
            dataType: "xml",
            url: "modules/mod_epoints/ecomm_clasific.jsp",
...
function RefreshClas1(obj, optClas) {
...
function loMasVendido() {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "xml",
        url: "modules/mod_epoints/ecomm_prod.jsp?Oper=2",
        success: function (datos) {
            ...
            for (var i = 0; i < lstProd.length; i++) {
                ...
                var factorPuntos = objCat.getAttribute('FactorPuntos');
                ...
                var Puntos1 = parseFloat(precio) / parseFloat(factorPuntos);
                var Puntos = Math.round(Puntos1);
                ...
                if (img2 == "/Sistema") {
                    strHtmlW += "<a href=\"" + img1 + "\" id=\"link1\" title=\"\">";
                } else {
                ...
                strHtmlW += "</a> ";
                strHtmlW += "</div> ";
                strHtmlW += "<div class=\"prod_txt\">" + descCat + "<br>";
                strHtmlW += "<br>Valor Puntos: " + FormatNumber(Puntos, intNumdecimal, true);
                strHtmlW += "</div> ";
                ...
                strHtmlW += "<input type=hidden id=lealtadCA_v" + intUnikiD + " name=lealtadCA" + intUnikiD + " value='" + Puntos + "'> ";
...
function loMasNuevo() {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "xml",
        url: "modules/mod_epoints/ecomm_prod.jsp?Oper=3",
        success: function (datos) {
            var lstXml = datos.getElementsByTagName("productos")[0];
            var lstProd = lstXml.getElementsByTagName("prod");
            //Variable para html
            ...
            for (var i = 0; i < lstProd.length; i++) {
                ...
                strHtmlW += "<input type=hidden id=lealtadCA_n" + intUnikiD + " name=lealtadCA" + intUnikiD + " value='" + Puntos + "'> ";
...
function listaProductos(strSearch) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "xml",
        url: "modules/mod_epoints/ecomm_proveedor.jsp?Oper=1",
        success: function (datos) {
            var lstXml = datos.getElementsByTagName("proveedor")[0];
            var lstProd = lstXml.getElementsByTagName("prov");
            //Variable para html
            ...
            for (var i = 0; i < lstProd.length; i++) {
                ...
                var PV_ID = objCat.getAttribute('PV_ID');
                var PV_RAZONSOCIAL = objCat.getAttribute('PV_RAZONSOCIAL');
                var PV_PAGINAWEB = objCat.getAttribute('PV_PAGINAWEB');
                var PV_FACEBOOK = objCat.getAttribute('PV_FACEBOOK');
                var PV_CONTACTO1 = objCat.getAttribute('PV_CONTACTO1');
                var PV_TELEFONO1 = objCat.getAttribute('PV_TELEFONO1');
                var PV_IMG1 = objCat.getAttribute('PV_IMG1');
                var PV_RESTRICCIONES = objCat.getAttribute('PV_RESTRICCIONES');
//                var PV_IMG1 = "images/epoints-logo.png";
                ...
                strHtmlW += "<div class=\"" + strStyle1 + "\"> ";
                strHtmlW += "<div class=\"prod_img\"> ";
                strHtmlW += "<a href=\"" + PV_IMG1 + "\" id=\"link1\" title=\"\">";
                strHtmlW += "<image src=\"" + PV_IMG1 + "\" border=\"0\" alt=\"\" title=\"\"> ";
                strHtmlW += "</a> ";
                strHtmlW += "</div> ";
                strHtmlW += "<div class=\"prod_txt\">" + PV_RAZONSOCIAL + " ";
                strHtmlW += "<br>" + PV_PAGINAWEB + " ";
                strHtmlW += "<br>" + PV_FACEBOOK + " ";
                strHtmlW += "<br>" + PV_CONTACTO1 + " ";
                strHtmlW += "<br>" + PV_TELEFONO1 + " ";
                strHtmlW += "<br>" + PV_RESTRICCIONES + " ";
                ...
                strHtmlW += "<input type=\"button\" class=\"ecomm_btn1\" " +
                        "name=\"add\" id=\"add" + i + "\" " +
                        " value=\"Buscar Productos\" onClick=\"ShowProductos(" + PV_ID + ")\"> ";
...
function ShowProductos(PV_ID) {
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "xml",
        url: "modules/mod_epoints/ecomm_proveedor.jsp?Oper=2",
        success: function (datos) {
            var lstXml = datos.getElementsByTagName("productos")[0];
            var lstProd = lstXml.getElementsByTagName("prod");
            //Variable para html
            ...
            for (var i = 0; i < lstProd.length; i++) {
                ...
                var activo = objCat.getAttribute('activo');
                var agrupa = objCat.getAttribute('agrupa');
                var restriccion = objCat.getAttribute('restriccion');
                var factorPuntos = objCat.getAttribute('FactorPuntos');
                //Obtenemos los precios
                var objPrec = objCat.getElementsByTagName("Precio")[0];
                var precio = 0;
                ...
                strHtmlW += "<br>Restricciones: " + restriccion;
                ...
                strHtmlW += "<input type=hidden id=lealtadCA" + intUnikiD + " name=lealtadCA" + intUnikiD + " value='" + Puntos + "'> ";
...
function AddItem(idProd, obj, iUid) {
    ...
    var PtosCte = document.getElementById("DISPONIBLES").value;


    var PtosTotal = document.getElementById("FAC_PUNTOS").value;

    var PtosProdAct = document.getElementById("lealtadCA" + strPref + iUid).value;
    var cantProd = document.getElementById("cant" + strPref + iUid).value;

    var PuntosProdActu = PtosProdAct * cantProd;

    var TotalPuntos = parseFloat(PtosTotal) + parseFloat(PuntosProdActu);

    if (TotalPuntos > PtosCte) {
        alert("Tus puntos no son suficientes Para agregar este producto");
        return false;
    } else {
        ...
        if (intAgrupa == 1 && parseFloat(idProd) == 0) {
            ...
            $.ajax({
                ...
                cache: false,
                dataType: "xml",
                url: "modules/mod_epoints/ecomm_prod.jsp?Oper=4",
                success: function (datos) {
                    var lstXml = datos.getElementsByTagName("productos")[0];
                    var lstProd = lstXml.getElementsByTagName("prod");
...
//                alert("Puntos de lealtad..." + obj.pr_lealtad);
//                alert("Puntos de lealtad cambio......" + obj.pr_lealtadCA);
        ...
        if (bolPasa) {
//            alert("Puntos de lealtad..." + parseFloat(document.getElementById("lealtad" + strPref + iUid).value));
//            alert("Puntos de lealtad cambio......" + parseFloat(document.getElementById("lealtadCA" + strPref + iUid).value));
...
//    document.getElementById("Total_importe").innerHTML =   FormatNumber(dblSuma, intNumdecimal, true);
    ...
    document.getElementById("Total_puntos").innerHTML = FormatNumber(dblImportePto, intNumdecimal, true);
...
function guardarCompra() {
    ...
    for (var j = 0; j <= intContaItems; j++) {
        ...
        strHtml += "<td align='right'><input type=button onclick='BorraGrid(" + j + ")' value='Borrar'></td>";
        strHtml += "</tr>";
    }
...
//    document.getElementById("Total_importe").innerHTML = intTotal;
    ...
    document.getElementById("Total_puntos").innerHTML = 0.0;
    document.getElementById("ImportePuntos_flete_cobrar").value = 0.0;
    document.getElementById("ImportePuntos_total_cobrar").value = 0.0;
...
/**Continuamos en formas de pago*/
function Step4() {

    var PtosCte = document.getElementById("DISPONIBLES").value;
    var PtosFlete = document.getElementById("ImportePuntos_total_cobrar").value;
    var PtosTotal = parseFloat(PtosFlete);

    if (PtosFlete != 0.0) {
        if (PtosTotal > PtosCte) {
            alert("Tus puntos no son suficientes para agregar el flete y transporte");

        } else {

            document.getElementById("FAC_PUNTOS").value = PtosTotal;
            document.getElementById("Total_puntos").innerHTML = PtosTotal;

            document.getElementById("TOTAL_PUNTOS_X_GASTAR").value = PtosTotal;
        ...
        alert("Seleccione alguna opciòn");
    }
}
...
function SaveVtaDo() {
    ...
    var dblImporteFlete = 0;
//    var dblImporteFlete = parseFloat(document.getElementById("Importe_flete_cobrar").value.replace(",", "").replace("$", ""));
    var dblImporte_ = parseFloat(document.getElementById("FAC_IMPORTE").value.replace(",", ""));
    var dblImporteTot_ = parseFloat(document.getElementById("FAC_TOT").value.replace(",", ""));
//    dblImporte_ += dblImporteFlete;
//    dblImporteTot_ += dblImporteFlete;
    //Master
    strPOST += "SC_ID=" + intSucursal;
    strPOST += "&CT_ID=" + intCte;
    strPOST += "&TOTAL_PUNTOS=" + document.getElementById("FAC_PUNTOS").value;
    strPOST += "&VE_ID=0";
    strPOST += "&PD_ID=0";
    ...
    strPOST += "&" + strPrefijoMaster + "_TOTAL=" + dblImporteTot_;
    strTotal = dblImporteTot_;
    ...
    strPOST += "&" + strPrefijoMaster + "_IMPORTE=" + dblImporte_;
    ...
    //Sumamos los fletes
    if (dblImporteFlete > 0) {
        intC++;
        strPOST += "&PR_ID" + intC + "=" + document.getElementById("PR_ID_FLETE").value;
        strPOST += "&" + strPrefijoDeta + "_EXENTO1" + intC + "=" + 1;
        strPOST += "&" + strPrefijoDeta + "_EXENTO2" + intC + "=" + 1;
        strPOST += "&" + strPrefijoDeta + "_EXENTO3" + intC + "=" + 1;
        strPOST += "&" + strPrefijoDeta + "_CVE" + intC + "=" + "FLETE";
        strPOST += "&" + strPrefijoDeta + "_DESCRIPCION" + intC + "=" + "ESTAFETA";
        strPOST += "&" + strPrefijoDeta + "_CANTIDAD" + intC + "=" + 1;
        strPOST += "&" + strPrefijoDeta + "_RET_ISR" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_RET_IVA" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_RET_FLETE" + intC + "=" + 1;
        //MLM
        strPOST += "&" + strPrefijoDeta + "_IMP_PUNTOS" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_IMP_VNEGOCIO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_PUNTOS" + intC + "=" + 0
        strPOST += "&" + strPrefijoDeta + "_VNEGOCIO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_DESC_PREC" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_DESC_PTO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_DESC_VN" + intC + "=" + 0;
        //      strPOST += "&" + strPrefijoDeta + "_DESC_LEAL" + intC + "=" + objItem.FACD_DESC_LEAL;
        strPOST += "&" + strPrefijoDeta + "_DESC_ORI" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_REGALO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_ID_PROMO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_PRECIO" + intC + "=" + dblImporteFlete;
        strPOST += "&" + strPrefijoDeta + "_PRECREAL" + intC + "=" + dblImporteFlete;
        strPOST += "&" + strPrefijoDeta + "_IMPORTE" + intC + "=" + dblImporteFlete;
        ...
        strPOST += "&" + strPrefijoDeta + "_NOSERIE" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_IMPORTEREAL" + intC + "=" + dblImporteFlete;
        strPOST += "&" + strPrefijoDeta + "_DESCUENTO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_PORDESC" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_ESDEVO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_PRECFIJO" + intC + "=" + 0;
        strPOST += "&" + strPrefijoDeta + "_NOTAS" + intC + "=" + "";
    ...
    $.ajax({
        ...
        cache: false,
        dataType: "html",
        url: "modules/mod_epoints/ecomm_save.jsp?id=1",
        success: function (dato) {

            var bolMercadoPago = false;

            dato = trim(dato);
            if (Left(dato, 3) == "OK.") {
                //Mostramos el numero de pedido generado
                var intFolio = dato.replace("OK.", "");

                document.getElementById("tabs-6").style.display = "block";
                document.getElementById("tabs-5").style.display = "none";
                //Imprimimos el pedido
                ImprimeconFolio(intFolio, dato, strNomFormat, bolMercadoPago);

//                    document.getElementById("FAC_PUNTOS").value = document.getElementById("FAC_PUNTOS").value TOTAL_PUNTOS_X_GASTAR;
                limpiarCompra();
            } else {
                alert(dato);
            }
...
function ImprimeconFolio(strKey, dato, strNomFormat, bolMercadoPago) {
    ...
    $.ajax({
        ...
        success: function (datos) {
            ...
            strPOST += "&PAYMENT_CLIENT_ID=" + document.getElementById("PAYMENT_CLIENT_ID").value;
            strPOST += "&PAYMENT_CLIENT_SECRET=" + document.getElementById("PAYMENT_CLIENT_SECRET").value;


            var objsc = datos.getElementsByTagName("vta_folios")[0];
            var strFolioT = objsc.getAttribute('FOLIO');
            var strHtml2 = CreaHidden("TKT_ID", dato.replace("OK.", ""));
            document.getElementById("IdPedidoWeb").innerHTML = "<h3>" + dato.replace("OK.", "") + "</h3>";
            openFormatEcomm("TICKET", "PDF", strHtml2);
...
function sumaTotalFleteTotal(dblValorFlete, dblFactor) {


//    document.getElementById("Importe_flete_cobrar").value = "$" + FormatNumber(dblValorFlete, 2, true);
//    document.getElementById("Importe_total_cobrar").value = "$" + FormatNumber(dblValorFlete + parseFloat(document.getElementById("FAC_TOT").value), 2, true);

    var dblFlete = dblValorFlete / dblFactor;
    var dblTotal = dblFlete + parseFloat(document.getElementById("FAC_PUNTOS").value);


    document.getElementById("ImportePuntos_flete_cobrar").value = FormatNumber(dblFlete, 2, true);
    document.getElementById("ImportePuntos_total_cobrar").value = FormatNumber(dblTotal, 2, true);

}

function BorraGrid(j) {


    var cantidadProducto = 0;
    for (var l = 0; l <= intContaItems; l++) {
        var objItem = lstItems[l];
        if (j == l) {
            cantidadProducto = objItem.pr_cantidad;
        }
    }

    var lstItemsTemp = Array();
    for (var l = 0; l <= intContaItems; l++) {
        var objItem = lstItems[l];
        if (j != l) {
            alert(objItem);
            lstItemsTemp.push(objItem);
        }
    }
    lstItems = Array();
    for (var x = 0; x <= lstItemsTemp.length - 1; x++) {
        var objItem = lstItemsTemp[x];
        lstItems.push(objItem);
    }
    intContaItems = lstItems.length - 1;
    for (var l = 0; l <= intContaItems; l++) {
        var objItem = lstItems[l];
    }
    intTotPzas = intTotPzas - cantidadProducto;
