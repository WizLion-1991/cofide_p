   ...
   function upfilemlm() {
      ...
      if (File.value == "") {
         ...
         if (File.value.toUpperCase().substring(File.value.length - 3, File.value.length) == "PDF") {
            $.ajaxFileUpload({
               url: 'modules/mod_capitalia/upfilemulti.jsp',
               secureuri: false,
         ...
         <div style="text-align: center">
            <table cellpadding="10" cellspacing="5" border="5" >
               <%                        Fechas fecha = new Fechas();
                  String strSql1 = "select PROS_FOLIO,LIS_ID, CT_ID,LIS_FECHA  FROM mlm_listado where CT_ID =  " + varSesiones.getintIdCliente();
                  String PROS_FOLIO = "";
                  ...
                  while (rs.next()) {
                     PROS_FOLIO = rs.getString("PROS_FOLIO");

                     LIS_ID = rs.getString("LIS_ID");
                     CT_ID = rs.getString("CT_ID");
                     LIS_FECHA = rs.getString("LIS_FECHA");
               %>
               <tr>
                  <td><font size=2><%=LIS_ID%></font></td>
                  <td><font size=2><%=PROS_FOLIO%></font></td>
