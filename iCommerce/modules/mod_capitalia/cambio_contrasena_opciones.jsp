    ...
    if (varSesiones.getIntNoUser() != 0 && seg.ValidaURL(request)) {
        ...
        if (strid != null) {
            if (strid.equals("1")) {
                ...
                String strSQL = "Select * From vta_cliente Where CT_ID =" + varSesiones.getIntNoUser();
                ResultSet rs = oConn.runQuery(strSQL);
                String strPass = "";
                while (rs.next()) {
                    strPass = rs.getString("CT_PASSWORD");
                    System.out.println(strPass+":"+strCntAnt);
                    if (strPass.equals(strCntAnt)) {
                        Captcha captcha = (Captcha) session.getAttribute(Captcha.NAME);
                        if (captcha.isCorrect(strAnswer)) {
                            strResult = CC.CambiaContrasenia(oConn, varSesiones.getIntNoUser(), strPassword);
