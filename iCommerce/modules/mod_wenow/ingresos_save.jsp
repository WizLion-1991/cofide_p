...
<%
    ...
    if (varSesiones.getIntNoUser() != 0) {
        ...
        String strKit_ingreso = request.getParameter("kit");
        String strAnswer = request.getParameter("answer");

        int intIdinvito = varSesiones.getIntNoUser();
        String strCurp = request.getParameter("curp");

        String intTelefRecar = "";
        //try {

        intTelefRecar = request.getParameter("telefonorecargar");
    //  } catch (NumberFormatException ex) {

        //     System.out.println("Error al convertir telefonorecargar...");
        // }
        String strCompaniaTelef = request.getParameter("compania_telefono");
...
/*         try {
            ...
            int intCompaniaTelef = 0;
            //obtenemos La compania del telefono
            strSql = "select COMPTEL_ID from prosp_inf_compania_telefono where  COMPTEL_NOMBRE= '" + strCompaniaTelef + "'";
            try {
                rs2 = oConn.runQuery(strSql, true);
                while (rs2.next()) {
                    intCompaniaTelef = rs2.getInt("COMPTEL_ID");
                }
                rs2.close();
            } catch (SQLException ex) {
                System.out.println("ERROR:" + ex.getMessage());
            ...
            //obtenemos El nombre de la persona que ingreso
            strSql = "select ct_razonsocial from vta_cliente where ct_id = '" + intIdinvito + "'";
            try {
                rs2 = oConn.runQuery(strSql, true);
                while (rs2.next()) {
                    strNomInvito = rs2.getString("ct_razonsocial");
                }
                rs2.close();
            } catch (SQLException ex) {
                System.out.println("ERROR:" + ex.getMessage());
            ...
            objTabla.setFieldInt("CT_SPONZOR", varSesiones.getintIdCliente());
            ...
            objTabla.setFieldString("CT_CATEGORIA1", strKit_ingreso);

            
            if (strKit_ingreso.equals("1")) {
                objTabla.setFieldString("WN_TRAINING", "1");
            }
            if (strKit_ingreso.equals("2")) {
                objTabla.setFieldString("WN_AFILIADO", "1");
            }
            if (strKit_ingreso.equals("3")) {
                objTabla.setFieldString("WN_GLOBAL", "1");
            }
            if (strKit_ingreso.equals("4")) {
                objTabla.setFieldString("WN_PREFERENTE", "1");
            }
            ...
            objTabla.setFieldInt("CT_ID_INVITO", intIdinvito);
            objTabla.setFieldString("CT_CURP", strCurp);
            objTabla.setFieldString("CT_TELEFONO_RECARGAR", intTelefRecar);
            objTabla.setFieldInt("CT_COMPANIA_TEL_RECA", intCompaniaTelef);
            objTabla.setFieldString("CT_NOMBRE_INVITO", strNomInvito);
