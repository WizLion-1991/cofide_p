...
<%
    ...
    if (varSesiones.getIntNoUser() != 0) {
        ...
        String strTIngreso = request.getParameter("tingreso");
        ...
        String strIFE = "";//request.getParameter("ife").trim();

        String strTestigo1 = request.getParameter("modlgn-Testigo1");
        String strTestigo2 = request.getParameter("modlgn-Testigo2");
        String strCodigoPromo = request.getParameter("mdlgn-codigo_promo");
        String strCiudadNac = request.getParameter("modlgn-ciudad_nac");
        String strCiudadReside = request.getParameter("modlgn-ciudad_residencia");
        String strBeneficiario = request.getParameter("modlgn-beneficiario");
        String strResult = "";
        int intDigito = 0;
        String strKey = "";
        int intPr_Id = 0;
        ...
        if (captcha.isCorrect(strAnswer)) {
            ...
            rs.close();
            int intSponsor = varSesiones.getintIdCliente();
            /*try {
            ...
            objTabla.setFieldString("CT_CONTACTO1", strBeneficiario);
            ...
            objTabla.setFieldString("CT_TESTIGO1", strTestigo1);
            objTabla.setFieldString("CT_TESTIGO2", strTestigo2);
            objTabla.setFieldString("CT_CODIGO_PROMO", strCodigoPromo);
            objTabla.setFieldString("CT_CIUDAD_NACI", strCiudadNac);
            objTabla.setFieldString("CT_CIUDAD_RESI", strCiudadReside);
            ...
            if (strCodigoPromo != "") {
                String strQuery = "select *  from mlm_codigo_promocion where  MCP_ACTIVO = '1' and MCP_USADO = '0'  and MCP_CODIGO_PROMOCION = '" + strCodigoPromo + "' ";
                try {
                    ResultSet rs1 = oConn.runQuery(strQuery);
                    while (rs1.next()) {
                        existsCodigo = true;
                    }
                    rs1.close();
                ...
                if (existsCodigo) {
                    String strUpdateCod = "update mlm_codigo_promocion set MCP_USADO = 1 where MCP_CODIGO_PROMOCION = '" + strCodigoPromo + "';";
                    oConn.runQueryLMD(strUpdateCod);
                    //Generamos una alta
                    strResult = objTabla.Agrega(oConn);
                    strKey = objTabla.getValorKey();
                    int intMlm = 0;
                    String strMlm = "select CT_MULTIPLE from vta_cliente where CT_ID = '" + intSponsor + "';";
                    try {
                        ResultSet rs1 = oConn.runQuery(strMlm);
                        while (rs1.next()) {
                            intMlm = rs1.getInt("CT_MULTIPLE");
                        }
                        rs1.close();
                    } catch (SQLException ex) {
                        System.out.println(ex.getMessage());
                    ...
                    if (intMlm == 0) {
                        ...
                        activador.setIntUplineInicial(intSponsor);
                        activador.setIntUplineTemporal(4);
                        activador.activarDistribuidor(Integer.parseInt(objTabla.getValorKey()));
                    } else {
                        objTabla.setFieldInt("CT_UPLINE", intSponsor);
                        objTabla.Modifica(oConn);
                    }
                } else {
            ...
            if (strResEntrega.equals("OK")) {
                ...
                if (!strEmail1.isEmpty() || !strEmail2.isEmpty()) {
                    ...
                    //Adjunta el CONTRATO al correo
                    String strContrato = generaContrato(Integer.parseInt(objTabla.getValorKey()), strRazonSocial, oConn);

                    File f = new File(strContrato);
                    if (f.exists()) {
                        mail.setFichero(strContrato);
                    }
                ...
                //Mail referenciado
                GeneraMailReferenciado(oConn, varSesiones, Integer.valueOf(objTabla.getValorKey()), varSesiones.getintIdCliente(), 1);
            }
            oConn.close();
        } else {
...
// #################### Funciones   ####################
    ...
    public String GeneraMailReferenciado(Conexion oConn, VariableSession varSesiones, int intNvoKey, int intSponzor, int contador) {
        String strRes = "";
        String strEmail1 = "";
        String strEmail2 = "";
        int intCT_ID = 0;
        //Buscamos los datos del sponzor
        String strSqlUsuarios = "select CT_ID, CT_SPONZOR, CT_RAZONSOCIAL, CT_EMAIL1, "
                + "CT_EMAIL2 from vta_cliente where CT_ID = " + intSponzor;
        try {
            ResultSet rs = oConn.runQuery(strSqlUsuarios);

            while (rs.next()) {
                intCT_ID = rs.getInt("CT_ID");
                int intSponzor2 = rs.getInt("CT_SPONZOR");
                strEmail1 = rs.getString("CT_EMAIL1");
                strEmail2 = rs.getString("CT_EMAIL2");
                if (contador <= 4) {
                    contador++;
                    GeneraMailReferenciado(oConn, varSesiones, intNvoKey, intSponzor2, contador);
                }
            }
        ...
        if (!strEmail1.isEmpty() || !strEmail2.isEmpty()) {
            ...
            mail.getMensaje();
            String strSqlEmp = "SELECT *,"
                    + "(select b.CT_RAZONSOCIAL from vta_cliente b where b.CT_ID = " + intSponzor + ") "
                    + "AS CT_RAZONSOCIAL_UPLINE"
                    + " FROM vta_cliente"
    ...
    public String generaContrato(int intIdCt, String strRazonSocial, Conexion oConn) {
        strRazonSocial = strRazonSocial.replace(" ", "_");

        Map parametersMap = new HashMap();
        parametersMap.put("CT_ID", intIdCt);
        String strPathBase = this.getServletContext().getRealPath("/");
        String strSeparator = System.getProperty("file.separator");

        String jrxmlFileName = "rep_contrato_ingreso_mlm.jrxml";
        String jasperFileName = "rep_contrato_ingreso_mlm.jasper";
        String pdfFileName = "contrato_" + strRazonSocial + ".pdf";

        String strReportPath = "";
        if (strPathBase.endsWith("/")) {
            strReportPath = strPathBase + "tmp"
                    + strSeparator;
        } else {
            strReportPath = strPathBase + strSeparator + "tmp"
                    + strSeparator;
        }
        //Generamos el pdf
        try {
            JasperCompileManager.compileReportToFile(strReportPath + jrxmlFileName, strReportPath + jasperFileName);
            // Generate jasper print
            JasperPrint jprint = (JasperPrint) JasperFillManager.fillReport(strReportPath + jasperFileName, parametersMap, oConn.getConexion());
            // Export pdf file
            JasperExportManager.exportReportToPdfFile(jprint, strReportPath + pdfFileName);
        ...
        return strReportPath + pdfFileName;
    }
